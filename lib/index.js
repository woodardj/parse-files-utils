...
function getAllFileObjects() {
  ...
  return schemas.get().then(function(res){
    ...
    var schemasWithFiles = onlyFiles(res).filter(function(s){ return (s.className === "TB_TourPhoto")});
    console.log("Schemas with Files:");
    console.log(schemasWithFiles);
    console.log("#===========#");
    return Promise.all(schemasWithFiles.map(getObjectsWithFilesFromSchema));
  }).then(function(results) {
    console.log("I think we don't make it here");
...
function getObjectsWithFilesFromSchema(schema) {
  ...
  return getAllObjects(query).then(function(results) {
    console.log('gotAllObjects: ' + schema.className);
